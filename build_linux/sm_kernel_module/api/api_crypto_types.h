...
typedef struct hash_t {
  unsigned char bytes[LENGTH_HASH];
} hash_t;

typedef struct secret_key_seed_t {
  unsigned char bytes[LENGTH_SEED];
} key_seed_t;

typedef struct public_key_t {
  unsigned char bytes[LENGTH_PK];
} public_key_t;

typedef struct secret_key_t {
  unsigned char bytes[LENGTH_SK];
} secret_key_t;

typedef struct signature_t {
  unsigned char bytes[LENGTH_SIG];
} signature_t;

typedef struct symmetric_key_t {
  unsigned char bytes[LENGTH_SYM_KEY];
} symmetric_key_t; 
